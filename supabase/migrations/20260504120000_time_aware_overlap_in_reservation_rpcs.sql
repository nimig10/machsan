-- Time-aware overlap in reservation RPCs
--
-- Bug: create_reservation_v2 and create_lesson_reservations_v1 used a
-- date-only overlap check (borrow_date <= NEW.return_date AND
-- return_date >= NEW.borrow_date). For lessons that share a date but
-- run at different hours (e.g. lecturer A teaches Tue 09:00-11:00,
-- lecturer B teaches the same Tue 12:45-15:30), the RPC reported the
-- equipment as fully reserved even though the time windows did not
-- overlap. The frontend's getAvailable() in src/utils.js already
-- checks time-aware overlap, so the UI showed "available: 1" while
-- submission failed with "not enough units" — confusing & incorrect.
--
-- Fix: replace the date-range check with a tstzrange overlap that
-- combines borrow_date + borrow_time and return_date + return_time
-- in Asia/Jerusalem, mirroring the JS logic exactly. Endpoints are
-- inclusive on the start and exclusive on the end so back-to-back
-- bookings (one ends 11:00, next starts 11:00) do NOT collide —
-- this matches `bStart < resEnd && rEnd > resStart` in JS.
--
-- Available_units recompute at the bottom of each RPC is unchanged
-- (it's the "tied up right now" snapshot, not future overlap).
...
BEGIN
  ...
  -- Match the JS getAvailable() fallback: missing time → all-day window.
  ...
  LOOP
    ...
    -- Time-aware overlap: builds a tstzrange for each existing reservation
    -- using its own borrow/return date+time (with the same all-day fallback
    -- as the JS) and checks tstzrange overlap with the new request.
    -- '[)'  = inclusive start, exclusive end → back-to-back bookings OK.
