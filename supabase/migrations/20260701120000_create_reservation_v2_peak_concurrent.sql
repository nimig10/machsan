-- Availability at submission = PEAK-CONCURRENT demand, not a naive SUM.
--
-- THE BUG THIS CLOSES:
--   create_reservation_v2 (and the approval guard) computed reserved demand as
--   SUM(ri.quantity) over every blocking reservation whose window overlaps the
--   new [borrow, return] window. When the new window SPANS several blocking
--   loans that do NOT overlap EACH OTHER, the sum over-counts: two 1-unit loans
--   in disjoint windows only ever tie up ONE physical unit at a time (units are
--   fungible — the same returned unit serves the next borrower). A 2-unit item
--   with two disjoint 1-unit loans was reported as 0 available for any window
--   spanning both, blocking legitimate requests.
--
-- THE FIX:
--   Reserved demand = the MAXIMUM simultaneous quantity in use at any instant
--   within [v_new_start, v_new_end). The minimum number of units needed to serve
--   a set of intervals equals their maximum overlap (interval-graph result), so
--   available = healthy_units − peak_concurrent_demand. Peak concurrency is always
--   reached at some blocker's start (clamped to the window start), so we evaluate
--   the running demand only at those candidate points.
--
-- ⚠️ ANTI-REGRESSION: this re-declares create_reservation_v2 BYTE-FOR-BYTE on
-- 20260623120200 (the current authoritative definition). All FOUR guards are
-- PRESERVED unchanged:
...
--   4. external-loan restriction (פרטית / הפקה)
-- The ONLY change is the v_reserved_count computation: SUM → peak-concurrent.
-- The blocking-status set, the overlap window and the '[)' range semantics are
-- IDENTICAL — keep them in sync with update_reservation_status_v1.
...
BEGIN
  ...
  LOOP
    ...
    --
    -- PEAK-CONCURRENT, not SUM: take the MAX simultaneous demand at any instant
    -- within [v_new_start, v_new_end). Blocking loans in disjoint windows do not
    -- add up — one physical unit serves consecutive non-overlapping loans. Peak
    -- is always reached at some blocker's start (clamped to v_new_start), so we
    -- evaluate the running demand only at those candidate points.
    WITH blk AS (
    ...
    SELECT COALESCE(MAX(c), 0) INTO v_reserved_count
      FROM (
