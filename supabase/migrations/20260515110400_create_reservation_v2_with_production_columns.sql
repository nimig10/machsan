-- Productions board (לוח הפקות) — Session 5/N: extend create_reservation_v2
-- to pass through production_id + production_date_id from p_reservation.
-- All overlap math, locking, status-init, and available_units recompute
-- are byte-identical to `20260504120000_time_aware_overlap_in_reservation_rpcs.sql`.
-- Only diff: the INSERT column list now includes the two new FK columns,
-- and the VALUES clause reads them from p_reservation (NULL when absent).
