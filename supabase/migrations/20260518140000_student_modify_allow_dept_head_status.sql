-- Expand student_modify_reservation_item_v1 to accept 'אישור ראש מחלקה' status.
-- Production loans pass through dept-head approval; the student/director must
-- still be able to remove items or cancel the whole reservation while waiting
-- for the dept head. The RPC previously only allowed 'ממתין' or 'מאושר'.
