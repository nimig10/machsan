-- Belt-and-suspenders for the production crew snapshot.
--
...
-- crew's NAMES, because the warehouse cert-gate (getProductionCertBlockers)
-- reads crew_photographer_name / crew_sound_name off the reservation row.
--
-- A client race (certifications.students not yet loaded when the production was
-- picked) could submit a production reservation with EMPTY crew names, which
-- left it stuck in "דרושה הסמכה לפני אישור" for every cert-required item even
-- though the production had an approved photographer + sound. The client fix
-- re-derives the snapshot at submit time; this RPC change is the safety net:
-- when production_id is present and a crew name arrives empty, derive it from
-- production_crew here so the snapshot is always correct regardless of client.
--
-- ONLY the crew columns change. Availability / overlap logic is byte-for-byte
-- the same as 20260516160000 (still blocks ONLY מאושר / באיחור / פעילה).
