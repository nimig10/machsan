-- Pending requests don't block inventory
--
-- Bug: lecturer in the lecturer portal could not request equipment that
-- a student had already requested in the public form (status="ממתין")
-- — the RPC counted "ממתין" as reserved and reported "not enough units",
-- even though the student's request was still tentative and could be
-- denied or never approved.
--
-- The intended business rule (per product owner): only RESERVATIONS THAT
-- ACTUALLY TIE UP THE EQUIPMENT block a new request — that is
-- {מאושר, פעילה, באיחור}. Pending statuses (ממתין, אישור ראש מחלקה)
-- are tentative and must NOT count against availability. Staff sees
-- pending overlaps separately via getReservationApprovalConflicts at the
-- moment they go to approve a pending request, so double-booking is
-- still surfaced — but at the right step in the workflow.
--
-- Frontend's getAvailable() in src/utils.js was updated to match in the
-- same change; both code paths now agree on the blocking-statuses set.
-- The regression test is updated separately to lock this contract.
...
BEGIN
  ...
  LOOP
    ...
    -- Only approved/active/overdue reservations actually tie up inventory.
    -- Pending requests (ממתין, אישור ראש מחלקה) are tentative and ignored.
...
BEGIN
  ...
  FOR v_reservation IN SELECT * FROM jsonb_array_elements(p_reservations) LOOP
    ...
    LOOP
      ...
      -- Only approved/active/overdue reservations tie up inventory.
      -- Pending statuses ignored — they're tentative.
      SELECT COALESCE(SUM(ri.quantity), 0) INTO v_reserved_count
        FROM public.reservation_items ri
        JOIN public.reservations_new r ON r.id = ri.reservation_id
        WHERE ri.equipment_id = v_equipment_id
...
-- Replace the regression test to lock in the new contract: pending
-- statuses MUST NOT count as reserved. Adds two new scenarios:
--   - a "ממתין" (pending) reservation in the same window → reserved=0
--   - an "אישור ראש מחלקה" reservation in the same window → reserved=0
-- Existing 11 scenarios kept as-is. The test data is rebuilt with two
-- extra pending rows on a third equipment item so the new scenarios
...
DECLARE
  ...
  v_eq_id_c       TEXT := '__test_overlap__eq_C';
  ...
  v_unit_id_c1    TEXT := '__test_overlap__unit_C1';
  ...
  v_res_pending   TEXT := '__test_overlap__res_pending';
  v_res_dh_pending TEXT := '__test_overlap__res_dh_pending';
  v_actual        INT;
...
BEGIN
  ...
  INSERT INTO public.equipment (id, name, available_units) VALUES
    ...
    (v_eq_id_c, '__test_overlap__ ציוד C (1 יח׳, pending only)', 1);
  ...
  -- New: a pending student-style request on equipment C (12:45-15:30)
  INSERT INTO public.reservations_new (id, status, borrow_date, borrow_time, return_date, return_time)
    VALUES (v_res_pending, 'ממתין', '2026-09-01', '12:45', '2026-09-01', '15:30');
  INSERT INTO public.reservation_items (reservation_id, equipment_id, name, quantity)
    VALUES (v_res_pending, v_eq_id_c, '__test_overlap__', 1);

  -- New: a "אישור ראש מחלקה" (department head approval) request on equipment C, different day
  INSERT INTO public.reservations_new (id, status, borrow_date, borrow_time, return_date, return_time)
    VALUES (v_res_dh_pending, 'אישור ראש מחלקה', '2026-09-02', '10:00', '2026-09-02', '14:00');
  INSERT INTO public.reservation_items (reservation_id, equipment_id, name, quantity)
    VALUES (v_res_dh_pending, v_eq_id_c, '__test_overlap__', 1);
  ...
  INSERT INTO _test_cases (label, expected, eq_id, new_start, new_end) VALUES
    ...
    ('eqA same window: cancelled+returned ignored, only active counted',
        ...
        ('2026-08-04'::date + '12:45'::time) AT TIME ZONE 'Asia/Jerusalem',
        ('2026-08-04'::date + '15:30'::time) AT TIME ZONE 'Asia/Jerusalem'),
    -- NEW scenarios: pending reservations must NOT count as reserved
    ('eqC same window as a ממתין pending request — must NOT block',
        0, v_eq_id_c,
        ('2026-09-01'::date + '12:45'::time) AT TIME ZONE 'Asia/Jerusalem',
        ('2026-09-01'::date + '15:30'::time) AT TIME ZONE 'Asia/Jerusalem'),
    ('eqC same window as an אישור-ראש-מחלקה pending request — must NOT block',
        0, v_eq_id_c,
        ('2026-09-02'::date + '10:00'::time) AT TIME ZONE 'Asia/Jerusalem',
        ('2026-09-02'::date + '14:00'::time) AT TIME ZONE 'Asia/Jerusalem');

  FOR scenario, expected_reserved, v_loop_eq_id, v_actual IN
    SELECT
      tc.label,
      tc.expected,
      tc.eq_id,
