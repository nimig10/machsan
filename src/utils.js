...
// storageGet / storageSet / keepAlive / autoBackup / sanity-threshold /
// hebrew-corruption / restoreCacheValue helpers removed: store table dropped
// (migration 20260430220000). Every entity now reads/writes via dedicated
// table APIs in src/utils/<entity>Api.js.

// hasHebrewCorruption / storageSet removed: store table dropped (migration
// 20260430220000). Every entity write now goes through src/utils/<entity>Api.js
// or a dedicated /api/* endpoint (createReservation, sync-equipment, etc.).
...
export const INITIAL_EQUIPMENT = [
  ...
  { id:5, name:"מיקרופון Rode NTG3",     category:"מיקרופונים",  description:"מיקרופון שוטגאן",            total_quantity:8, image:"", notes:"",                    status:"תקין" },
  { id:6, name:"מיקרופון DJI Mic 2",     category:"מיקרופונים",  description:"מיקרופון לבלבי אלחוטי",      total_quantity:10,image:"", notes:"",                    status:"תקין" },
  { id:7, name:"מקליט Zoom H6",          category:"מקליטי אודיו",description:"מקליט שדה 6 ערוצים",         total_quantity:4, image:"", notes:"לכלול סוללות",        status:"תקין" },
...
// ─── LOAN TYPE COLORS ────────────────────────────────────────────────────────
// Fixed palette assigning a unique color to each loan type. Used across
// PublicForm mini-calendar, Dashboard calendar, and LecturerPortal/DeptHead
// calendar so a student/lecturer/staff instantly recognizes loan types by color.
// Format: [backgroundRgba, textColor].
export const LOAN_TYPE_COLORS = {
  "פרטית":       ["rgba(52,152,219,0.75)",  "#fff"],  // כחול
  "הפקה":        ["rgba(230,126,34,0.75)",  "#fff"],  // כתום
  "סאונד":       ["rgba(26,188,156,0.75)",  "#fff"],  // טורקיז
  "קולנוע יומית": ["rgba(155,89,182,0.75)",  "#fff"],  // סגול
  "שיעור":       ["rgba(241,196,15,0.75)",  "#000"],  // צהוב
  "צוות":        ["rgba(149,165,166,0.75)", "#fff"],  // אפור
  "איש צוות":    ["rgba(149,165,166,0.75)", "#fff"],  // אפור (alias)
};
export const PREVIEW_COLOR = ["rgba(245,166,35,0.45)", "#f5a623"];
export function getLoanTypeColor(type) {
  return LOAN_TYPE_COLORS[String(type || "").trim()] || ["rgba(127,140,141,0.6)", "#fff"];
}
