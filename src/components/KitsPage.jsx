      ...
      {mode===null&&lessonKits.length>0&&(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {lessonKits.map(kit=>{
              ...
              const linkedLesson = kit.lessonId ? lessons.find(ls=>String(ls.id)===String(kit.lessonId)) : null;
              return (
                <div key={kit.id} style={{background:"var(--surface)",border:"1px solid rgba(155,89,182,0.3)",borderRadius:"var(--r)",padding:"16px 18px"}}>
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                      ...
                      <div>
                        <div style={{fontWeight:800,fontSize:15}}>{kit.name}</div>
                        {linkedLesson&&<div style={{fontSize:12,color:"#9b59b6",marginTop:2,fontWeight:700}}>📽️ {linkedLesson.name}{linkedLesson.instructorName?` · ${linkedLesson.instructorName}`:""}</div>}
                        {!linkedLesson&&kit.instructorName&&<div style={{fontSize:12,color:"var(--text2)",marginTop:2}}>👨‍🏫 {kit.instructorName}{kit.instructorPhone?` · 📞 ${kit.instructorPhone}`:""}</div>}
                        {kit.instructorEmail&&<div style={{fontSize:11,color:"var(--text3)"}}>✉️ {kit.instructorEmail}</div>}
                        <div style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap"}}>
                          {(kit.schedule||[]).length>0&&<span style={{background:"rgba(155,89,182,0.15)",border:"1px solid rgba(155,89,182,0.35)",borderRadius:20,padding:"2px 8px",fontSize:11,color:"#9b59b6",fontWeight:700}}>
