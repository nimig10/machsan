...
export function ReservationsPage({ reservations, setReservations, equipment, showToast,
  ...
  const deleteReservation = async (id) => {
    ...
    // Virtual lesson reservations (auto-generated by buildLessonReservations)
    // have no DB row. Cancel by flagging the lesson session — lesson schedule
    // and kitId stay intact so the lesson itself is NOT modified.
    // Lecturer can recreate the loan from their portal.
    if (res?.lesson_auto) {
      if (!setLessons) {
        showToast("error", "לא ניתן למחוק בקשת שיעור כאן");
        return;
      }
      const lessonId = String(res.lesson_id);
      const kitId = String(res.lesson_kit_id || "");
      const borrowDate = String(res.borrow_date || "");
      let flagged = false;
      const updatedLessons = lessons.map((l) => {
        if (String(l.id) !== lessonId) return l;
        const schedule = Array.isArray(l.schedule) ? l.schedule : [];
        const nextSchedule = schedule.map((s) => {
          if (flagged) return s;
          const dateMatch = String(s?.date || "") === borrowDate;
          const kitMatch =
            (kitId && String(s?.kitId || "") === kitId) ||
            (!s?.kitId && String(l?.kitId || "") === kitId);
          if (dateMatch && kitMatch) {
            flagged = true;
            return { ...s, cancelledRequest: true };
          }
          return s;
        });
        return { ...l, schedule: nextSchedule };
      });
      if (!flagged) {
        showToast("error", "לא נמצא מפגש מתאים לבקשה");
        return;
      }
      ...
      try {
        ...
        logActivity({
          user_id: caller.id, user_name: caller.full_name,
          action: "lesson_loan_cancel", entity: "lesson", entity_id: lessonId,
          details: { borrow_date: borrowDate, kit_id: kitId, instructor: res?.student_name },
        }).then((logId) => onLogCreated(logId)).catch(() => {});
      } catch (err) {
        console.error("lesson loan cancel failed:", err);
        showToast("error", "הביטול נכשל — נסה שוב");
      }
      return;
    }
