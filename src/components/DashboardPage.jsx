              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:"var(--text3)"}}>
                  ...
                  <div style={{marginTop:3,display:"flex",flexDirection:"column",gap:3}}>
                    <span style={{display:"inline-flex",alignItems:"center",flexWrap:"wrap",gap:6}}><Calendar size={14} strokeWidth={1.75} color="var(--accent)" /><span style={{fontWeight:800,color:"var(--text2)"}}>הוצאה:</span><span style={{color:"var(--text)",fontWeight:700}}>{formatDate(r.borrow_date)}</span>{r.borrow_time&&<strong style={{color:"var(--accent)"}}>{formatTime(r.borrow_time)}</strong>}</span>
                    <span style={{display:"inline-flex",alignItems:"center",flexWrap:"wrap",gap:6}}>↩<span style={{fontWeight:800,color:"var(--text2)"}}>החזרה:</span><span style={{color:"var(--text)",fontWeight:700}}>{formatDate(r.return_date)}</span>{r.return_time&&<strong style={{color:"var(--accent)"}}>{formatTime(r.return_time)}</strong>}</span>
                  </div>
                </div>
              </div>
      ...
      {dashViewRes&&(
        <div className="dash-quickview-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 16px"}} onClick={e=>e.target===e.currentTarget&&setDashViewRes(null)}>
          ...
          <div className="dash-quickview-modal" style={{width:"100%",maxWidth:640,background:"var(--surface)",borderRadius:18,border:"1px solid var(--border)",direction:"rtl",maxHeight:"92vh",overflowY:"auto",overflowX:"hidden"}}>
            {/* Header — sticky, so whatever lives here stays reachable while the
                equipment list scrolls. Laid out as rows: title+close on top, the
            ...
            <div className="dash-quickview-header" style={{display:"flex",flexDirection:"column",gap:10,minWidth:0,padding:"18px 22px",borderBottom:"1px solid var(--border)",background:"var(--surface2)",borderRadius:"18px 18px 0 0",position:"sticky",top:0,zIndex:1}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,minWidth:0}}>
                ...
                {/* Same square icon button the shared Modal uses (ui.jsx).
                    flex-start, not center, so it lines up with the name rather than
                    floating mid-way down a two-line block. */}
              ...
              {/* Gated on the RAW status: getEffectiveStatus would read an
                  ...
                  Rendered only when editable, so a returned request keeps the
                  shorter two-line header instead of an empty row. */}
              {["ממתין","מאושר","נדחה","באיחור"].includes(dashViewRes.status)&&(
                <div className="dash-quickview-actions" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                  <button className="btn btn-secondary" style={{padding:"9px 22px"}} title="עריכת הבקשה"
