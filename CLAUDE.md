...
> 📱 **חריג זמני — תקופת מובייל (מ-2026-05-31, ~3 שבועות):** למשתמש אין גישה ל-localhost (עובד מהטלפון). בתקופה זו הבדיקה הידנית של **שינויי קוד** עוברת מ-Stage 1 (localhost) ל-**Stage 2 (Vercel Preview)**: פותחים **PR ייעודי**, והמשתמש בודק ומאשר על ה-Preview (מחובר ל-dev DB) לפני merge ל-main. **שאר חוקי הברזל ללא שינוי** — שינויי DB/schema/migration עדיין dev-first ובאישור מפורש, ואין merge בלי אישור המשתמש.
