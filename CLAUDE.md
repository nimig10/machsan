...
> מסמך הקשר יחיד לסשנים חדשים. סנאפ-שוט עדכני נכון ל-**2026-05-26** (PR #24 פתוח, לא ממוזג).
...
- `src/App.jsx` — shell מרכזי (~7,471 שורות). מכיל orchestration גלובלי (state, routing, realtime, auth bootstrap) + **8 דפים inline** שעוד לא חולצו.
- `src/components/LessonsPage.jsx` — ~4,078 שורות (PR #24 הוסיף עמודות מרצה מרובות + lecturerIds[] לכל מפגש).
...
### 📱 אופציה לסשני ענן בלבד (mobile-only)
רלוונטי **רק** בסשנים שנפתחים פה ב-Claude Code on the web (טלפון בלבד, בלי מחשב). **לא מחליף ולא משנה את הזרימה הקבועה למעלה** — תוספת אופציונלית בלבד:
- **Stage 1 מוחלף ב-Stage 2**: כשאין גישה ל-localhost, **Vercel Preview (על dev DB) הוא משטח הבדיקה הידנית** של המשתמש. המשתמש פותח את ה-Preview URL בדפדפן בטלפון, בודק ידנית ומאשר במפורש שעובד — בדיוק כמו דרישת Stage 1.
- בדיקה ידנית של המשתמש לפני פרוד **נשארת חובה** (על ה-Preview במקום localhost). `npm run test:db`/CI נשארים עזר בלבד.
- מיגרציה לפרוד + merge ל-main עדיין דורשים **אישור מפורש לסשן**. ברירת מחדל: פיתוח על ענף feature, לא דחיפה ישירה ל-main בלי רשות.
...
### N מרצים למפגש + `course_lecturers` JSONB column (PR #24 — פתוח, לא ממוזג)
...
**מיגרציה `20260526200000_lessons_course_lecturers.sql`** (הוחלה ב-dev בלבד, **לא ב-prod**):
```sql
ALTER TABLE public.lessons
  ADD COLUMN IF NOT EXISTS course_lecturers jsonb NOT NULL DEFAULT '[]'::jsonb;
```
שורה אופציונלית עם DEFAULT — אין נזק לrows קיימים. צריך להחיל ב-prod **לפני merge** של PR #24.
...
- **2026-05-26** — **PR #24** (**פתוח, ממתין לבדיקה ידנית**) — N עמודות מרצה למפגש (`session.lecturerIds[]`) + כפתורי "+ / − הוסף/הסר עמודת מרצה". XL import: `findAllH("מרצה",...)` תופס "מרצה 1/2/3" כעמודות נפרדות, merge key ללא lecturer. עמודה חדשה `lessons.course_lecturers jsonb` (מיגרציה `20260526200000`, **dev בלבד**). תיקון multi-classroom import bundled. LecturerPortal filter מתקן מרצים משניים. **מיגרציה צריכה להיות מוחלת ב-prod לפני merge.**
