...
- **ICS smoke** (`npm run test:ics`, [scripts/run-ics-smoke.mjs](scripts/run-ics-smoke.mjs)) — 9 בדיקות על חוזה קובץ היומן (PR #81): `METHOD:PUBLISH` בלי `ORGANIZER`/`ATTENDEE`/`SEQUENCE`, UID לכל VEVENT, קיפול ≤75 אוקטטים, round-trip base64, `escParam`, `COLLEGE_ADDRESS` בגרשיים עבריים בלי ASCII `"`, `LOCATION` בלי שם חדר, ואיסור `encoding` מפורש על חלק היומן. **בלי רשת ובלי DB.** status נוכחי: **9/9 PASS**. כל בדיקה כאן מקבעת כשל אמיתי שקרה — ראה לקח #38.
