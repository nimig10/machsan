...
> מסמך הקשר יחיד לסשנים חדשים. סנאפ-שוט עדכני נכון ל-**2026-06-13** (סוף סשן — אחרי PR #46–#47). **PR #46**: `production_crew_change_recheck_v1` מרענן crew snapshot על ההזמנות כשצוות מאושר/משתנה אחרי הגשת רשימת ציוד + backfill (מיגרציה `20260613150000`). **PR #47**: שחזור per-student overlap guard גלובלי ב-`create_reservation_v2` שהופל ב-PR #45 (מיגרציה `20260613153000`) + פאנל צף בשלב האישור + `getEffectiveStatus` גוזר "באיחור" (מקור-אמת יחיד, סוף הקפיצה פעילה↔באיחור) + טסט CI `run_student_overlap_tests`. baseline קודם: PR #44–#45 (crew snapshot ב-`create_reservation_v2` `20260604120000` + re-seed זהות), PR #39–#43 (מובייל + `HH:MM` + מייל דדליין הפקה + `no-undef`=error).
...
- **בדיקות overlap**: `assert_reservation_overlap_ok`, `run_reservation_overlap_tests`, `run_student_overlap_tests` (5 תרחישים — per-student guard, PR #47).
...
- **DB smoke** (`npm run test:db`, [scripts/run-db-smoke.mjs](scripts/run-db-smoke.mjs)) — 24 scenarios: `run_reservation_overlap_tests` (13) + `run_productions_regression_tests` (6) + `run_student_overlap_tests` (5). מסרב לרוץ אם ה-hostname לא `mhvujejdlmtowypjdhjd`. status נוכחי: **24/24 PASS**.
