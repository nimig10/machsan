...
> **מסמך ההקשר היחיד לסשנים חדשים.** עדכני ל-**2026-08-21** (אחרי PR #123 — מוזג; #121–#123 הם code-only, בלי מיגרציות).
>
...
`checkout_outcomes` jsonb (אותו חוזה, לחריגי **ההוצאה**; `יוצא` לעולם לא נשמר) ·
`issued_by_staff_id`+`issued_by_name` (מי מסר את הציוד) ·
**`issued_at`** — ⚠️ **החריג היחיד לכלל ה-display-only**: בורר-הפאנל ושער עריכת-הסטודנט (לקח #51) ·
...
- `פעילה` — ציוד יצא לסטודנט בפועל. **ערך אמיתי ב-DB מאז מסך ההוצאה** — עד אז הוא היה נגזרת-קליינט בלבד שאף קוד לא כתב (לקח #51).

**לא חוסמות**: `ממתין`, `אישור ראש מחלקה`, `נדחה`, `הוחזר`, `בוטל`.

> ⚠️ **`לא יצא?` הוא תווית-תצוגה שכן חוסמת** — הוא לא ערך ב-DB (השורה נשארת
> `מאושר`/`באיחור`, וזה מה ש-`create_reservation_v2` סופר), ולכן כל חישוב זמינות
> בקליינט חייב לספור אותו כמו `באיחור`. ראה לקח #53.
> **הטריגר שלו הוא מועד ה_איסוף_ + חלון חסד (ברירת מחדל 30 דק׳, `notPickedUpGraceMinutes`
> בהגדרות מערכת) — לא מועד ההחזרה.** לכן הוא מופיע גם על שורות שמועד החזרתן בעתיד,
> ו**חלון ה-48h לא חל עליהן**. ראה לקח #58.
...
19. **`getEffectiveStatus` הוא מקור-האמת היחיד לסטטוס מוצג, ו-`normalizeReservationsForArchive` חייב לגזור **בדיוק אותו דבר**** — PublicForm טוען שורות גולמיות ל-state המשותף ש-App מנרמל, ואי-הסכמה בין השניים גרמה לקפיצה פעילה↔באיחור. שניהם גוזרים היום **רק** `מאושר`/`פעילה` + עבר מועד ההחזרה → `באיחור` (שיעורים מוחרגים). **`פעילה` כבר לא נגזר ואסור להחזיר את הגזירה** — הוא נכתב ל-DB בהוצאה בפועל בלבד (לקח #51); גזירה-מחדש הייתה מייצרת `פעילה` רפאים לכל בקשה מאושרת שמועד האיסוף שלה עבר, ומעלימה את מסך ההוצאה בדיוק מהבקשות שזקוקות לו.
...
51. **`פעילה` = איש מחסן מסר ציוד ביד, ו-`issued_at` הוא מה שקובע איזה מסך מוצג — לא הסטטוס** — `באיחור` נושא מאז שתי משמעויות (“בחוץ ואיחר” ו”מעולם לא נאסף”) ולכן `checkoutState()` הוא הבורר היחיד. **`issued_at` הוא החריג היחיד לכלל “חותמת = display-only”**: הוא נכתב ב-PATCH **חוסם לפני ה-RPC** (מסונן `issued_at=is.null`, 0 שורות = "already" ולא שגיאה) ו**אסור להעבירו ל-`stampActor`** — כשל שקט שם, ואז הקרון שמעביר ל-`באיחור`, היה מציג מסך הוצאה לציוד שכבר יצא ומקטין את רשימת הפריטים פעם שנייה. סדר הכתיבות **מלאי → פריטים → סטטוס** נגזר מכלל אחד: **כל prefix חייב להיות שמרני-יתר לגבי מלאי** — דקרמנט לפני כתיבת היחידות היה משחרר כמות **וגם** משאיר יחידה שבורה כ-`תקין`. **`computeCheckoutItems` הוא רשימת-יעד מוחלטת** (`newQty` = כמה סומנו `יוצא`), והפאנל **נזרע מ-`original_items ?? items`** — דלתא או זריעה מהרשימה החיה מכווצות את הבקשה בכל ניסיון חוזר. `החזר` הוא **המצב היחיד שמשנה כמות**; `פגום`/`נעלם` מתועדים בלבד. `pickUnitsForCheckout` מקבל `include` כדי שניסיון חוזר ישאל על **אותן** יחידות ולא ישרוף יחידה תקינה שנייה. **אסור לעבור דרך `makeSaveEditedReservation`** — הוא שולח `original_items` תמיד וה-RPC ממפה `jsonb 'null'` ל-SQL NULL, כלומר מוחק חותמת קיימת. **ו-`checkoutWindowOpen` הוא השער היחיד — אסור להוסיף override שעוקף אותו.** היה כפתור "הוצא עכשיו" שנתן להוציא ציוד ימים מראש, והנזק לא היה בלבול אלא **ספירת מלאי שקרית**: `computeEquipmentAvailability` **וגם** `create_reservation_v2` גוזרות את תחילת חלון-החסימה מ-`borrow_date` ולא מ-`issued_at`, ולכן ציוד שיצא מוקדם נספר **זמין** בשתי השכבות בזמן שהוא בתיק של סטודנט — ואף גארד לא תופס את זה. סטודנט שמגיע מוקדם מטופל בהזזת מועד האיסוף ב"עריכת בקשה", שמתקנת **גם** את חלון המלאי. תקרת `checkoutLeadHours` היא 12 שעות בדיוק מהסיבה הזו (הייתה 72 = 3 ימים), והקלמפ ב-`commitNumber` חייב להסכים עם ה-`max` של השדה. `half_issued` **אינו** override אלא התאוששות מכתיבה שמתה באמצע, והוא יושב באותו `||` — אסור למחוק אותו יחד איתו.
52. **ביטול הגזירה פתח גייטים שהיו סגורים בטעות** — `student_modify_reservation_item_v1` **מעולם לא בדק זמן**; מה שחסם סטודנט ממחיקת פריטים או ביטול השאלה חיה היה **רק** `st==="פעילה"` בקליינט, כלומר הגזירה. עכשיו `issued_at IS NULL` נאכף **ב-RPC** (אותו טוקן `not editable` → 409 קיים, אפס שינוי ב-API) ובקליינט דרך `studentMayModifyItems` ב-`loanPolicy.js` — predicate **יחיד** ל-5 המשטחים ב-PublicForm, שקורא **`r.status` גולמי** כדי שהכפתור וה-RPC יסכימו. ובכיוון ההפוך: **`"פעילה"` חייב להיכנס לרשימות הסטטוס הגולמיות של הצוות** (כפתור "עריכת בקשה" ב-3 מקומות, `isOverdueReservation` ב-`EditReservationModal`, ותזכורת ה-push ב-`check-overdue.js`) — הן התאימו במקרה כל עוד אף שורה לא הייתה `פעילה` ב-DB.

53. **`באיחור` פוצל לשניים, ו-`לא יצא?` הוא תווית-תצוגה שעדיין חוסמת מלאי** — `באיחור` נשא שתי משמעויות ("בחוץ ואיחר" ו"מעולם לא נאסף") ושלח את המחסן לחפש ציוד שמונח על המדף. המבחין הוא `wasCheckedOut` דרך `lateStatusFor` — **אותו מבחין של `checkoutState`**, כדי שהתו והפאנל לא יחלקו על אותה שורה; `getEffectiveStatus` ו-**שני** העותקים של `normalizeReservationsForArchive` חייבים לקרוא לו ולא לקודד תווית קשיחה (לקח #19). **`באיחור` הוא גם מועמד לתיוג-מחדש ולא רק תוצאה** — הקרון חותם אותו בלי להסתכל על `issued_at`. **הסכנה האמיתית**: הערך לא קיים ב-DB (השורה נשארת `מאושר`/`באיחור`, וזה מה שה-RPCs סופרים), ולכן `computeEquipmentAvailability` **חייבת** לספור אותו — היא מסננת לפי `getEffectiveStatus`, והשמטתו הייתה מציגה מלאי פנוי שה-RPC דוחה ב-409 בלי הסבר. לכן `INVENTORY_BLOCKING_STATUSES` ו-`isOverdueLikeStatus` (כולל חלון ה-48h, זהה ל-`באיחור`) הם רשימה אחת משותפת ולא ליטרלים מפוזרים. **מיקום**: נשאר ברשימה הפעילה ולא בדף הדחויות — חלון ההוצאה שלו עדיין פתוח. **הקרון לא שולח לו מייל איחור** (`issued_at IS NULL`) — הנושא הוא "אזהרת איחור — נדרשת פעולה מיידית", והאשמה של מי שלא לקח כלום; ⚠️ **הגייט הזה עבר ל-pass נפרד ב-`check-overdue.js` וחייב להישאר שם**: מסנן ה-pass שמסמן `באיחור` מוציא שורות שכבר `באיחור`, ולכן שורה שסומנה בדקה 30 **לא חוזרת לרשימה לעולם** — מייל שיושב באותה לולאה עם דדליין מאוחר יותר פשוט לא נשלח, בלי שגיאה ובלי לוג. **שני הטיימרים נפרדים במכוון**: הסימון ל-`באיחור` הוא 30 דקות **קבועות** (איתות מוקדם לצוות, לא ניתן להגדרה), והמייל לסטודנט הוא `overdueEmailDelayMinutes` בהגדרות מערכת (ברירת מחדל **90**, `[0,1440]`) — חלון שנועד לתת לאיש המחסן לקלוט החזרה מורכבת לפני שהסטודנט מקבל נזיפה. ה-pass של המייל **אינו** דורש `status='באיחור'` (זה היה שובר בשקט כל ערך מתחת ל-30), והסורק בקליינט ב-`App.jsx` **חייב לקרוא את אותו מפתח** — הוא רץ בכל פעם שצוות פתוח באפליקציה, ו-30 דקות קשיחות שם מקדימות את הקרון ומבטלות את ההגדרה. זו גם **הקריאה היחידה ל-`site_settings` מתוך `api/`** — לא להעתיק את `readStoreKey` מ-`notify-course-end-7days.js`, הוא קורא שורת `store` שנמחקה ומחזיר `null` בשקט; החריגה מוצגת לצוות כתו ובכרטיסיית לוח-בקרה משלה. מחיר מודע: איש מחסן ששכח ללחוץ "הוצא" — ההשאלה תיראה לא-נאספת ולא יישלח מייל, והתו הוא מה שהופך את השלב שנשכח לגלוי. **ולכן `לא יצא?` אינו "הציוד על המדף" אלא "אף אחד לא לחץ", והוא ממשיך לחסום מלאי** — הסיכון א-סימטרי (עודף-חסימה = מטרד שמשתחרר לבד; חוסר-חסימה = המערכת מציגה "זמין" על ציוד שבתיק של סטודנט). **החלטת בעל המוצר, 2026-08-17.**
> ⚠️ **התווית היא קלט, לא רק פלט — `normalizeReservationsForArchive` דורסת בה את `status` בזיכרון לפני שהשורה מגיעה לרכיב.** לכן כל predicate שקורא `r.status` "גולמי" מקבל בפועל `לא יצא?`, וקריאתו כסטטוס לא-מוכר החזירה `none`: מסך ההוצאה, "הוצא עכשיו" ו"עריכת בקשה" נעלמו מהבקשה **היחידה** שחלון ההוצאה שלה עוד פתוח, ונשאר רק 🗑. `checkoutState` ו-`checkoutWindowOpen` מקפלים אותה בחזרה ל-`באיחור` (בטוח בבנייה: היא נפלטת רק כש-`issued_at IS NULL`), והרשימות **`STAFF_EDITABLE_STATUSES` / `INVENTORY_BLOCKING_STATUSES` הן משותפות ולא ליטרלים מפוזרים** — שתי רגרסיות נפרדות נולדו מרשימה inline שנשארה מאחור, כולל **`AIChatBot` שדיווח לסטודנטים מלאי-יתר**. `checkoutState` גם מחריג `שיעור`/`lesson_auto`, כמו שהמיגרציה כבר עושה ב-SQL.

54. **סימוני ההוצאה/החזרה חיים מעל המודאל, וממופתחים ב-`equipment_id` ולא במיקום במערך** — הפאנלים מרונדרים בתנאי בתוך המודאל, ולכן סגירה (כולל **לחיצה על הרקע**, קליק אחד תועה) עשתה unmount ו-React מחק את כל עבודת המשתמש. ה-store הוא ברמת-מודול ([useWarehouseMarks.js](src/hooks/useWarehouseMarks.js)) ולא state בכל דף — אחרת אותה בקשה שנפתחת מלוח הבקרה ומדף הבקשות מייצרת בדיוק את אותה תלונה במסלול השני. **אסור לזרוע `useState` מה-store** (StrictMode יורה effects פעמיים, ולקח #42); הקריאה סינכרונית ברינדור, והוולידציה של מה ששוחזר רצה שם — פריט שנעלם או **כמות שהשתנתה** נזרקים, כי סימון ירוק מצהיר "כל ה-N יוצאים". **המפתח חייב להיות יציב**: אף שאילתת `reservation_items` בריפו לא כוללת `ORDER BY`, ומפתח לפי אינדקס היה מדביק סימון לציוד אחר; הסיומת `#occurrence` מפרידה שתי שורות של אותו פריט. `getSnapshot` **חייב** להחזיר `EMPTY_DRAFT` הקפוא ולא `{}` טרי, וכל כתיבה מחליפה ולא מוטטת — אחרת React זורק/נכנס ללופ. חלון הפירוט **נגזר** (`detailOpen` הוא דגל-תצוגה בלבד; `exceptions === null` סימן גם "סגור" וגם "אין עבודה", וזו הייתה הסיבה ש"סגור" השמיד אותה), והמיפוי על הרשימה **המחודשת** *הוא* הגיזום — לכן `pickUnitsForReturn` **לא** מקבל `include` (החזרה לא תציע יחידה שיצאה ממחזור). **ניקוי רק בהצלחה** ורק ב-namespace של הזרימה — כישלון חייב לשמר את הסימונים ל-retry. **חובה להשאיר את "נקה סימונים"** — סגירת המודאל *הייתה* מנגנון הביטול, והשמירה מסירה אותו. **ומחיר הגזירה: חלון הפירוט חייב לקפוא ברגע ה-commit** (`busy && submittedRows`) — `completeEquipmentReturn`/`Checkout` קוראות ל-`setEquipment` **אופטימית לפני הרשת**, ולכן יחידה שסומנה `פגום` מפסיקה להיות `תקין`, נופלת מה-picker, והשורה נגזרת מחדש על חומרה אחרת בברירת המחדל — המשתמש רואה את הסימון שלו מתבטל לבד לאורך בקשת HTTP שלמה. הסנאפ-שוט נלקח **ב-onClick ולא ברינדור** (ref ברינדור מפיל את `react-compiler`), ומשתחרר כש-`busy` נגמר — בכישלון שכן כתב מלאי, רשימת היחידות החדשה *היא* האמת.

55. **כחול = הציוד במחסן, ירוק = הציוד בחוץ — ולשני הפאנלים אסור לחלוק צבע-סימון** — פאנל ההוצאה וההחזרה כמעט זהים בצורה (אותם כרטיסים, אותה לחיצה-לסימון, אותו חלון פירוט), ולכן הצבע הוא הדבר היחיד שאומר לאיש המחסן במבט אם הוא מוסר או מקבל. שניהם סימנו בירוק, והם התבלבלו בפועל. לכן `OUTCOME_COLOR['תקין']` הוא **`var(--blue)`** (ההחזרה כחולה לכל אורכה — כרטיס, צ'יפ, כפתור, ו-"🔵 הוחזר" בארכיון), `CHECKOUT_COLOR['יוצא']` נשאר ירוק, ו-`החזר` בהוצאה כחול כי גם הוא נשאר במחסן. **`פגום` אדום ו-`נעלם` סגול משותפים לארכיון ואסור לגעת בהם**; `תקין` לא מגיע לארכיון לעולם (לקח #48) ולכן הוא של הפאנל בלבד. ה-prop של הכרטיס הוא **`marked` ולא `green`** — אוצר-המילים `greenKeys`/`toggleWarehouseGreen` נשאר ב-store (מקובע ב-markDraft), אבל prop ששמו צבע שהוא לא צובע הוא בדיוק איך ששני הפאנלים נסחפו למראה זהה.

56. **`public.students` הוא המקור היחיד לשם של סטודנט — אין שדה שם חופשי בשום מסלול כתיבה** — שדות `שם פרטי`/`שם משפחה` בטופס ההשאלה היו input חופשי, הערך שרד בטיוטת `sessionStorage`, ובורר סוג-ההשאלה **לא איפס אותו** — ולכן שם שהוקלד בטופס אחד **דלף** להשאלה מסוג אחר שבה הכרטיס רק *נראה* קריאה-בלבד, ומשם נצרב ל-`reservations_new.student_name`. **31 שורות, 3 סטודנטים**, והנזק תפקודי ולא קוסמטי: חיפוש הצוות (`student_name?.includes`), **שער ההסמכות** (`matchByNamePhone` מתאים לפי שם מנורמל, ולכן ההסמכות מפסיקות להיספר והאישור נחסם בשקר), ו-`api/check-overdue.js` שקורא את השם מה-DB למייל ולגוף ההתראה. **התיקון ב-DB ולא בתצוגה** — תצוגה חיה היא 34 קבצים ו-~80 נקודות רינדור, ובעיקר **לא יכולה** להגיע ל-4 הצרכנים בשרת ולא ל-`crew_*_name` (עמודה בלי מייל לפתור לפיו). לכן: `api/create-reservation.js` **מקנוניקל מהרוסטר** לפני ה-RPC (`email=eq.` מדויק — **לעולם לא `ilike`**, כי `_` הוא wildcard של תו בודד), ושינוי שם ברובריקה **מקמפל** לבקשות (לפי מייל), ל-`crew_photographer_name`/`crew_sound_name` (**לפי `production_crew.student_id`, לעולם לא לפי השם הישן — הוא בדיוק מה שאי אפשר לסמוך עליו**), לאולפנים ולהפקות. הסלקטורים **אידמפוטנטיים בבנייה** (`!== nextName`), אחרת ה-autosave של 700ms מייצר לולאה. **`student_name` הוסר מ-`p_fields`** ב-`reservationEdit.js` — אין שדה שם במודאל, זה היה רק הד של מה שהיה בזיכרון, ולקח #22 מריץ `saveEditedReservation` **לפני כל אישור**, כך שכל אישור היה מקפיא מחדש שם ישן ומבטל את הקסקייד. **`night_*` ו-`equipment_reports` מוחרגים במכוון** — סנאפ-שוטי זהות ששורדים מחיקת סטודנט (לקח #37). *(פער פתוח: שינוי **מייל** ברובריקה עדיין מנתק את כל ההיסטוריה.)*

57. **`original_items` נחתם מחדש בעריכת צוות — וזה המסלול היחיד שמותר לו** — היפוך מודע של לקח #35+#44. איש מחסן חייב יכולת לתקן השאלה שכבר יצאה (הוא נזכר שמסר פריט נוסף), אבל **הרשימה שמופיעה בארכיון נגזרת מ-`original_items`**, ולכן עריכה שלא חותמת מחדש הייתה מציגה בסטטוס `הוחזר` את הרשימה שלפני התיקון. לכן `save()` ב-`EditReservationModal` חותם מחדש **כשהשורה כבר יצאה בלבד**, וההקלה מגודרת בגארדים **שליליים**: `checkoutApi.js` עדיין חותם פעם אחת (`alreadyStamped ? {} : {...}`) ו-`returnApi.js` **אינו כותב לא `original_items` ולא `reservation_items`**. בנוסף, תקרת הכמות בעריכה היא `getAvail(eqId)` **בלבד** — `getEquipmentBlockingDetails` כבר מעביר את `reservation.id` כ-`excludeId`, ולכן הוספת `originalQtyFor` הייתה סופרת את ההחזקה של הבקשה עצמה פעמיים. מאגר הפריטים הוא `catalogueEq` = הציוד החי ∪ שורות `original_items` שהציוד שלהן נמחק, אחרת פריט שהוסר מהמלאי נעלם מהעריכה של השאלה שעוד מחזיקה אותו. **התנאי לחתימה-מחדש הוא `isOverdueReservation || wasCheckedOut(reservation)`** — הוספת `wasCheckedOut` מכסה את `half_issued` (`issued_at` נכתב, הסטטוס נשאר `מאושר` כי הכתיבה מתה באמצע): הציוד בחוץ ועריכה חייבת להגיע לארכיון. ⚠️ **אסור להרחיב את `isOverdueReservation` עצמו** — הוא מזין 9 מקומות באותו קובץ, מהם **6 UI** ואחד שער הזמינות, והרחבתו מזיזה את מראה המודאל. כשדגל מזין גם לוגיקה וגם UI — מרחיבים בתנאי נפרד, לא בדגל.

58. **`לא יצא?` נגזר ממועד ה_איסוף_ + חלון חסד, ו-48h במלאי מגויט על מועד ה_החזרה_** — התווית נועדה להתריע שאיש צוות לא ביצע את ההוצאה, אבל היא חלקה טריגר עם `באיחור` (רק `issued_at` הבחין ביניהם) ולכן נדלקה במועד **ההחזרה**: השאלה שנאספת ב-18:00 ומוחזרת ב-23:30 נשארה `מאושר` עד 23:30, ובהשאלה שבועית ההתראה הגיעה **שבוע** באיחור. עכשיו `pickupNeverHappened` הוא **המימוש היחיד** של הטריגר ושלושת המשטחים קוראים לו — `getEffectiveStatus` ו**שני** עותקי `normalizeReservationsForArchive`; **אסור לשכפל בלוק שני**, הזוג הזה כבר נסחף פעם על `צוות` וזה מחיר לקח #19. **השער הוא `wasCheckedOut` ולא `issued_at` ישירות** — `פעילה` נחשב שיצא גם בלי חותמת, וזה גם מה ששומר את הקיפול ב-`checkoutState` בטוח-בבנייה (התווית נפלטת רק כשהציוד על המדף). **⚠️ הבאפר של 48h חייב `nowMs >= endTs` ולא את התווית לבדה**: מרגע שהיא מופיעה במועד האיסוף היא נוחתת על שורות שמועד החזרתן **בעתיד** ושה-RPC רואה כ-`מאושר` רגיל, והצמדת הבאפר אליהן הייתה מסתירה 48 שעות של מלאי שה-DB מחשיב פנוי — **הכיוון ההפוך של לקח #53, ובלי שום 409 שיסביר**. no-op לכל שורת `באיחור` אמיתית, שהיא מעבר ל-`endTs` בהגדרה. **הסטודנט לא רואה את התווית** — מקופלת ל-`"מאושר"` ב-`ActiveLoanCard`, ל**ליטרל** ולא ל-`r.status` שהוא כבר התווית עצמה (לקח #53); הקיפול יושב בתצוגה כדי שמשטחי הצוות, המלאי והארכיון ישמרו גזירה אחת. **כפתור המחיקה עובר `mayDeleteNotPickedUp`** ולא את התווית החשופה — מחיקה קשיחה בלי שחזור (לקח #47) אסור שתיפתח על השאלה חיה שהסטודנט רק מאחר אליה. **ואסור שהטריגר יזלוג ל-`api/check-overdue.js`** — הוא כותב `באיחור` ל-**DB**, וגזירה מ-`borrow_date` הייתה חותמת סטטוס אמיתי על כל השאלה שהמחסן מאחר בה בחצי שעה. חלון החסד ב-`notPickedUpGraceMinutes` (0–240, ברירת מחדל 30), והרגיסטר מסונכרן **ברינדור ולא ב-`useEffect`** — effect רץ אחרי הפאס, ושום דבר לא היה מרנדר מחדש כדי לתקן שורות שכבר נורמלו בערך הישן. **החלטות בעל המוצר, 2026-08-21.**
...
- **Student-identity tests** (`npm run test:identity`, [scripts/run-student-identity-tests.mjs](scripts/run-student-identity-tests.mjs)) — 100 בדיקות על [src/utils/studentIdentity.js](src/utils/studentIdentity.js), 75 טהורות ו-25 סריקות סטטיות. המרכזית: `selectReservationIdsForRename` בוחר **לפי מייל בלבד** — יש בדיקה שלילית מפורשת ששורה הנושאת את השם הישן אך מייל של מישהו אחר **אינה** נבחרת, כי השם הישן הוא בדיוק הערך שהוכח כלא-אמין. בנוסף: כל הסלקטורים **אידמפוטנטיים** (הזנת השורות שאחרי השינוי מחזירה `[]` — זה מה שעוצר לולאת autosave), `resolveRosterName` מחזיר `""` בלי התאמה ו**לעולם לא שם של סטודנט אחר**, `nameNeedsCascade` לא מרוקן שם ולא מגיב להפרש-רווחים, ו-`normalizeNameKey` **זהה בית-בית** לשני העותקים החיים (זו ההוכחה ששער ההסמכות לא מנרמל אחרת מהרוסטר). הסריקות מקבעות שאין שדה שם חופשי בקליינט ולא ב-endpoint, שאין `.ilike("email"` בשני הקבצים שתוקנו, ו**גארד היקף שלילי**: אין אזכור ל-`night_*`/`equipment_reports` (לקח #37). **בלי רשת ובלי DB.** status נוכחי: **100/100 PASS**.
...
- **Return-flow tests** (`npm run test:return`, [scripts/run-return-flow-tests.mjs](scripts/run-return-flow-tests.mjs)) — 78 בדיקות על [src/utils/returnFlow.js](src/utils/returnFlow.js), הלוגיקה של מסך החזרת הציוד. זו הזרימה השוטפת **היחידה שכותבת למלאי**, ולכן מקובע בעיקר מה שהורס: `applyUnitOutcomes` מחזיר את **מערך הציוד המלא** (מערך חלקי מוחק את היחידות של כל פריט שהושמט — לקח #21), לא מוסיף/מוריד יחידות, ולא נוגע ביחידה שלא נכללה ב-outcomes. בנוסף: `pickUnitsForReturn` בוחר `תקין` בלבד, דטרמיניסטית ומיון מספרי (#10 אחרי #9), לעולם לא יותר מהכמות המושאלת ולעולם לא ממציא יחידה; `fault` נשמר רק על `פגום`; והמודול נשאר חסר-תלויות. **44 מהן על סנאפ-שוט הארכיון** (לקח #48), והחשובה בהן היא ש-`readReturnOutcomes` מחזיר `null` לכל שורה שקדמה לפיצ'ר — המסלול של כל הארכיון בפרוד. 7 האחרונות על [markDraft.js](src/utils/markDraft.js) בצד ההחזרה, והחשובה בהן: יחידה שנפסלה במסך אחר **לא מוצעת ולא מוחייה** ע"י סימון שמור, ו-`pickUnitsForReturn.length === 2` מקבע שלא הוסיפו לה `include` (לקח #54). **בלי רשת ובלי DB.** status נוכחי: **85/85 PASS**.
- **Checkout-flow tests** (`npm run test:checkout`, [scripts/run-checkout-flow-tests.mjs](scripts/run-checkout-flow-tests.mjs)) — 101 בדיקות על [src/utils/checkoutFlow.js](src/utils/checkoutFlow.js). זו הזרימה היחידה שכותבת למלאי **וגם** עורכת את הבקשה באותה פעולה, ולכן מקובע בעיקר מה שמכווץ בקשה בשקט: `computeCheckoutItems` הוא **רשימת-יעד מוחלטת** — החלה כפולה על אותם outcomes נותנת אותה תוצאה, ויש בדיקה מפורשת שהזנת הרשימה **המוקטנת** בחזרה כן מכווצת שוב (זה מה שמסביר למה הזריעה היא `original_items ?? items`). בנוסף: `applyCheckoutOutcomes` מחזיר את **מערך הציוד המלא** (לקח #21) ו-`יוצא`/`החזר` משאירים את היחידה `תקין` · רק `החזר` מוריד כמות · `pickUnitsForCheckout` מכבד `include` ולעולם לא ממציא unit id · `checkoutState` על ארבעת המצבים כולל `פעילה` עם `issued_at` NULL · החלון נשאר פתוח **לנצח** אחרי מועד ההשאלה · וסריקה סטטית ש-`getEffectiveStatus` **לא** מחזיר `"פעילה"` באף ענף. **31 נוספו** על [markDraft.js](src/utils/markDraft.js) ועל שני הלקחים החדשים: היפוך סדר שורות **לא** משנה מפתח-סימון (לקח #54), סימון שכמותו שונתה נזרק, `INVENTORY_BLOCKING_STATUSES` סופר את `לא יצא?` ו-`computeEquipmentAvailability` ניגשת אליו דרך הרשימה המשותפת ולא דרך ליטרל משלה (לקח #53). **12 האחרונות סורקות את שני הפאנלים סטטית** ומקבעות את קיפאון-ה-commit של חלון הפירוט (לקח #54): הסנאפ-שוט נלקח ב-`onClick` ולא ברינדור, אין `useRef` באף אחד מהם, `outcomes` עדיין נגזר מ-`detailRows`, ו-`setEquipment` בשני ה-API עדיין רץ **לפני** `writeEquipmentToDB` — כי אם הכתיבה תיהפך פסימית, הקיפאון הופך למשקל מת וזו הבדיקה שתגלה זאת. **ועוד 6** על מוסכמת הצבע של לקח #55: `CHECKOUT_COLOR['יוצא'] !== OUTCOME_COLOR['תקין']`, כל אחד מהערכים בנפרד, ושלא נשאר ליטרל ירוק בפאנל ההחזרה. **ו-12 האחרונות על התווית-כקלט** (לקח #53): `checkoutState`/`checkoutWindowOpen` מול שורה **מנורמלת**, יציבות ה-fixed-point של `lateStatusFor`, החרגת שיעורים, ושלוש סריקות סטטיות שהרשימות המשותפות (`STAFF_EDITABLE_STATUSES`, `INVENTORY_BLOCKING_STATUSES` ב-`allApproved` וב-`AIChatBot`) לא חזרו להיות ליטרלים. **ו-12 האחרונות מקבעות שה-override של "הוצא עכשיו" לא חוזר** — הכפתור נמחק כי הוא עקף את `checkoutWindowOpen` לגמרי, וציוד שיצא לפני `borrow_date` נספר **זמין** גם בקליינט וגם ב-RPC (שניהם גוזרים את חלון החסימה מ-`borrow_date` ולא מ-`issued_at`). הסריקות **משמיטות הערות לפני החיפוש** — התיעוד שמסביר את ההסרה מזכיר את שם הכפתור והיה מפיל את הגארד של עצמו. **ו-12 על עריכת צוות בהשאלה שיצאה** (לקח #57), כולל שני גארדים **שליליים** ש-`checkoutApi.js` עדיין חותם `original_items` פעם אחת ו-`returnApi.js` לא כותב אותו כלל. **ו-49 האחרונות על הטריגר החדש** (לקח #58): גבולות `pickupOverdue` בשני הכיוונים, קלמפ `[0,240]` של הרגיסטר, שהקיפול של `checkoutState`/`checkoutWindowOpen` **שרד** את הטריגר המוקדם, ושלושה גארדים שקשה להפריז בהם — שאין קריאה שנייה ל-`pickupOverdue` מחוץ ל-helper (זה מה שמונע מזוג הנרמולים לסחוף גם את הכלל החדש), שהבאפר של 48h מגויט על `nowMs >= endTs`, ו**גארד שלילי על הקרון** שהוא עדיין גוזר מ-`return_date` בלבד. **בלי רשת ובלי DB.** status נוכחי: **257/257 PASS**.
